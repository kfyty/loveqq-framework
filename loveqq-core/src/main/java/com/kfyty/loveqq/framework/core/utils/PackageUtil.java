...
public abstract class PackageUtil {
    ...
    public static <T> List<T> scanInstance(Class<T> mainClass) {
        return scanInstance(mainClass, clazz -> !ReflectUtil.isAbstract(clazz) && mainClass.isAssignableFrom(clazz));
    }
    ...
    private static Set<String> scanClassNameByJar(URL url) {
        try {
            ...
            while (entries.hasMoreElements()) {
                ...
                classes.add(classPath.replace('/', '.').replace(".class", CommonUtil.EMPTY_STRING));
            }
            return classes;
        } catch (Exception e) {
            throw ExceptionUtil.wrap(e);
