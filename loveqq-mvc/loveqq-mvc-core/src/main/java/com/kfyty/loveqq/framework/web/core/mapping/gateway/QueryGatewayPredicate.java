...
public class QueryGatewayPredicate implements GatewayPredicate {
    /**
     * 配置
     */
    ...
    public boolean test(Route route, ServerRequest request) {
        ...
        if (regexp == null || regexp.isEmpty()) {
            return request.getParameterMap().containsKey(this.config.getParam());
        }
        String[] split = request.getParameter(this.config.getParam()).split(",");
